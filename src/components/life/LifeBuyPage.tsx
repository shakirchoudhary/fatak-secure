    ...
    <div className="pt-16 bg-slate-50 min-h-screen" ref={topRef}>
      ...
      <div className="bg-white border-b border-slate-100 px-[5%] py-3.5 flex items-center justify-between sticky top-16 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-heading font-bold text-navy-DEFAULT text-sm">Life Insurance</div>
            <div className="text-[11px] text-slate-400">{STEP_LABELS[currentStep]} · Step {currentStep + 1} of 9</div>
          </div>
        </div>
        {screen >= 3 && (
          <div className="flex items-center gap-1.5 text-xs font-semibold text-navy-DEFAULT bg-life-bg border border-life-DEFAULT/20 px-3 py-1.5 rounded-xl">
            <span>🛡️</span>
      ...
      {screen === 0 && (
        <div className="max-w-[480px] mx-auto px-5 py-10 text-center">
          ...
          <h2 className="font-heading text-[24px] font-extrabold text-navy-DEFAULT mb-2">Protect Your Family's Future</h2>
          ...
          <div className="bg-white rounded-2xl shadow-s1 p-6 text-left max-w-md mx-auto">
            ...
            <div className="flex items-center border-[1.5px] border-slate-200 rounded-xl overflow-hidden focus-within:border-life-DEFAULT focus-within:shadow-[0_0_0_3px_rgba(236,72,153,0.1)] transition-all mb-4">
              <div className="px-3.5 py-3 font-bold text-navy-DEFAULT bg-slate-50 border-r border-slate-200 text-sm flex-shrink-0">+91</div>
              <input
                type="tel"
                placeholder="Enter your mobile number"
                value={phone}
                onChange={e => setPhone(e.target.value.replace(/\D/g, '').slice(0, 10))}
                className="flex-1 px-3.5 py-3 border-none outline-none font-heading text-navy-DEFAULT text-sm bg-transparent"
              />
            </div>
            <div className="flex items-start gap-2.5 text-[12.5px] text-slate-500 leading-snug mb-5">
              <input type="checkbox" checked={consent} onChange={e => setConsent(e.target.checked)} className="mt-0.5 flex-shrink-0 accent-life-DEFAULT" />
              <label>I agree to the <span className="text-life-DEFAULT font-semibold cursor-pointer">Privacy Policy</span> and <span className="text-life-DEFAULT font-semibold cursor-pointer">Terms & Conditions</span></label>
            </div>
            <button
              disabled={phone.length !== 10 || !consent}
              onClick={() => goTo(1)}
              className="w-full py-3.5 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-life-DEFAULT to-life-dark text-white disabled:opacity-40 disabled:cursor-not-allowed hover:-translate-y-0.5 transition-transform shadow-[0_6px_22px_rgba(236,72,153,0.3)]"
            >
              Get Life Quotes →
            </button>
          </div>
            ...
            <div className="flex gap-3 justify-center mb-4">
              {otp.map((val, i) => (
                <input
                  ...
                  data-lotp={i}
                  className="w-14 h-16 border-2 border-slate-200 rounded-xl text-center font-heading text-2xl font-extrabold text-navy-DEFAULT outline-none focus:border-life-DEFAULT focus:shadow-[0_0_0_3px_rgba(236,72,153,0.12)] transition-all bg-white"
                />
              ))}
            </div>

            <p className="text-sm text-slate-400 mb-5">
              Didn't receive? <span className="text-life-DEFAULT font-semibold cursor-pointer" onClick={() => setOtp(['', '', '', ''])}>Resend OTP</span>
            </p>

            <button
              disabled={otp.join('').length !== 4}
              onClick={() => goTo(2)}
              className="w-full py-3.5 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-life-DEFAULT to-life-dark text-white disabled:opacity-40 disabled:cursor-not-allowed hover:-translate-y-0.5 transition-transform"
            >
              Verify & Continue →
            </button>
          </div>
      ...
      {screen === 2 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <h2 className="font-heading text-[22px] font-extrabold text-navy-DEFAULT mb-1.5">Tell us about yourself</h2>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-4">
            <h4 className="font-heading text-sm font-bold text-navy-DEFAULT mb-4">👤 Personal Details</h4>
            ...
            <input type="text" placeholder="Enter your full name" value={name} onChange={e => setName(e.target.value)}
              className="w-full px-3.5 py-3 border-[1.5px] border-slate-200 rounded-xl font-body text-sm text-navy-DEFAULT outline-none focus:border-life-DEFAULT transition-all bg-white mb-3.5" />
            ...
            <div className="grid grid-cols-2 gap-3 mb-3.5">
              <div>
                ...
                <input type="date" value={dob} onChange={e => setDob(e.target.value)}
                  className="w-full px-3.5 py-3 border-[1.5px] border-slate-200 rounded-xl font-body text-sm text-navy-DEFAULT outline-none focus:border-life-DEFAULT transition-all bg-white" />
              </div>
              <div>
                  ...
                  {(['Male', 'Female'] as const).map(g => (
                    <button key={g} onClick={() => setGender(g)}
                      className={`flex-1 py-3 rounded-xl text-sm font-bold border-[1.5px] transition-all cursor-pointer ${
                        gender === g ? 'border-life-DEFAULT bg-life-bg text-life-dark' : 'border-slate-200 bg-white text-slate-500'
                      }`}>
                      {g === 'Male' ? '♂ Male' : '♀ Female'}
                    </button>
                  ))}
              ...
              {[[true, '🚬 Yes'], [false, '✅ No']].map(([v, label]) => (
                <button key={String(v)} onClick={() => setSmoker(v as boolean)}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-bold border-[1.5px] transition-all cursor-pointer ${
                    smoker === v ? 'border-life-DEFAULT bg-life-bg text-life-dark' : 'border-slate-200 bg-white text-slate-500'
                  }`}>
                  {label as string}
                </button>
              ))}
          ...
          <div className="bg-white rounded-2xl shadow-s2 border border-slate-100 p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-sm font-bold text-navy-DEFAULT">Basic Info</div>
            </div>
            <button
              disabled={!name || !dob}
              onClick={() => goTo(3)}
              className="py-3 px-6 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-life-DEFAULT to-life-dark text-white disabled:opacity-40 disabled:cursor-not-allowed hover:-translate-y-0.5 transition-transform"
            >
              Continue →
            </button>
          </div>
      ...
      {screen === 3 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <h2 className="font-heading text-[22px] font-extrabold text-navy-DEFAULT mb-1.5">How much life cover do you need?</h2>
              ...
              {COVER_OPTIONS.map(c => (
                <div
                  ...
                  className={`border-2 rounded-2xl p-4 cursor-pointer transition-all text-center ${
                    cover === c.id ? 'border-life-DEFAULT bg-life-bg' : 'border-slate-200 bg-white hover:border-life-DEFAULT'
                  }`}
                >
                  <div className={`font-heading text-lg font-extrabold ${cover === c.id ? 'text-life-dark' : 'text-navy-DEFAULT'}`}>{c.label}</div>
          ...
          <button
            onClick={() => goTo(4)}
            className="w-full py-3.5 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-life-DEFAULT to-life-dark text-white hover:-translate-y-0.5 transition-transform"
          >
            Continue →
          </button>
        </div>
      ...
      {screen === 4 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <h2 className="font-heading text-[22px] font-extrabold text-navy-DEFAULT mb-1.5">For how many years do you need cover?</h2>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 text-center mb-4">
            ...
            <div className="flex items-center justify-center gap-5 py-5">
              <button onClick={() => changeTerm(-5)}
                className="w-11 h-11 rounded-full border-2 border-life-DEFAULT bg-white text-xl font-bold text-life-dark cursor-pointer flex items-center justify-center hover:bg-life-bg transition-all">−</button>
              <div>
                <div className="font-heading text-[48px] font-extrabold text-navy-DEFAULT leading-none">{term}</div>
              ...
              {[10, 15, 20, 30, 40].map(t => (
                <button key={t} onClick={() => setTerm(t)}
                  className={`px-3.5 py-1.5 rounded-xl text-sm font-semibold border-[1.5px] transition-all cursor-pointer ${
                    term === t ? 'border-life-DEFAULT bg-life-bg text-life-dark font-bold' : 'border-slate-200 bg-white text-slate-600'
                  }`}>
                  {t} yrs
                </button>
              ))}
      ...
      {screen === 5 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <h2 className="font-heading text-[22px] font-extrabold text-navy-DEFAULT mb-1.5">Who will receive the claim amount?</h2>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-4">
            <h4 className="font-heading text-sm font-bold text-navy-DEFAULT mb-4">👤 Nominee Details</h4>
          ...
          <div className="bg-white rounded-2xl shadow-s2 border border-slate-100 p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-sm font-bold text-navy-DEFAULT">{nomName || 'Not set'}</div>
            </div>
            <button
      ...
      {screen === 6 && (
        <div className="max-w-[480px] mx-auto px-5 py-24 text-center">
          <div className="relative w-[72px] h-[72px] mx-auto mb-5 border-[5px] border-life-bg border-t-life-DEFAULT rounded-full animate-spin" />
      ...
      {screen === 7 && (
        <div className="max-w-[480px] mx-auto px-5 py-6">
          {/* Profile banner */}
          <div className="bg-life-bg rounded-xl px-4 py-3 flex items-center gap-3 mb-4 border border-life-DEFAULT/20">
            <span className="text-xl">🛡️</span>
            <div>
              <div className="font-heading text-sm font-bold text-navy-DEFAULT">{gender} · {dob ? (new Date().getFullYear() - new Date(dob).getFullYear()) : 33} yrs · {smoker ? 'Smoker' : 'Non-smoker'}</div>
              <div className="text-xs text-slate-400">{COVER_LABELS[cover]} · {term} Year Term</div>
            </div>
            <button onClick={() => goTo(2)} className="ml-auto text-[11px] text-life-DEFAULT font-semibold cursor-pointer bg-transparent border-none">Edit ✏️</button>
          </div>
            ...
            {([['all', 'All Plans'], ['term', 'Pure Term'], ['rop', 'Return of Premium']] as const).map(([v, label]) => (
              <button key={v} onClick={() => setPlanFilter(v)}
                className={`px-4 py-1.5 rounded-2xl text-xs font-bold border-[1.5px] whitespace-nowrap cursor-pointer transition-all ${
                  planFilter === v ? 'border-life-DEFAULT bg-life-bg text-life-dark' : 'border-slate-200 bg-white text-slate-500 hover:border-life-DEFAULT'
                }`}>
                {label}
              </button>
            ))}
              ...
              return (
                <div
                  ...
                  className={`bg-white border-[1.5px] rounded-[18px] p-5 cursor-pointer transition-all duration-200 relative ${
                    isSelected ? 'border-life-DEFAULT shadow-s2' : 'border-slate-100 hover:border-life-DEFAULT'
                  }`}
                >
                  {p.popular && (
                    <div className="absolute -top-3 left-4 bg-gradient-to-r from-life-DEFAULT to-life-dark text-white text-[10.5px] font-bold px-3 py-1 rounded-xl">⭐ Recommended</div>
                  )}
                  <div className="flex items-center gap-3.5">
                    <div className="w-[50px] h-[50px] rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-[11px] font-extrabold text-navy-DEFAULT flex-shrink-0 text-center">
                      {p.short}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-heading text-[14.5px] font-bold text-navy-DEFAULT">{p.insurer}</div>
                    ...
                    <div className="text-right flex-shrink-0">
                      <div className="font-heading text-[20px] font-extrabold text-navy-DEFAULT leading-none">₹{p.base}</div>
                      <div className="text-xs text-slate-400">/year</div>
                    </div>
                  </div>
                  {isSelected && (
                    <div className="mt-4 pt-4 border-t border-slate-100 flex gap-2">
                      <button className="flex-1 py-2 rounded-xl border-[1.5px] border-life-DEFAULT text-life-dark font-heading font-bold text-xs bg-white cursor-pointer">View Details</button>
                      <button
                        onClick={e => { e.stopPropagation(); goTo(8) }}
                        className="flex-1 py-2 rounded-xl border-none cursor-pointer font-heading font-bold text-xs bg-gradient-to-br from-life-DEFAULT to-life-dark text-white hover:-translate-y-0.5 transition-transform"
                      >
                        Buy Now →
                      </button>
                    </div>
          ...
          <button
            ...
            onClick={() => goTo(8)}
            className="w-full mt-5 py-3.5 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-life-DEFAULT to-life-dark text-white disabled:opacity-40 disabled:cursor-not-allowed hover:-translate-y-0.5 transition-transform"
          >
            Continue →
          </button>
        </div>
      ...
      {screen === 8 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <h2 className="font-heading text-[22px] font-extrabold text-navy-DEFAULT mb-1.5">Enhance your coverage</h2>
          <p className="text-sm text-slate-500 mb-5">Add riders to your base plan for comprehensive protection.</p>

          <div className="bg-life-bg rounded-xl px-4 py-3 flex items-center gap-3 mb-5 border border-life-DEFAULT/20">
            <span className="text-xl">🛡️</span>
            <div>
              <div className="font-heading text-sm font-bold text-navy-DEFAULT">{plan.insurer}</div>
              <div className="text-xs text-slate-400">{plan.plan} · {COVER_LABELS[cover]} · {term} yrs</div>
            </div>
            <button onClick={() => goTo(7)} className="ml-auto text-[11px] text-life-DEFAULT font-semibold cursor-pointer bg-transparent border-none">Change</button>
          </div>

          {/* Mandatory PA */}
          <div className="bg-life-bg border border-life-DEFAULT/20 rounded-2xl p-4 flex items-start gap-3.5 mb-3">
            <div className="text-2xl flex-shrink-0">🧑‍⚕️</div>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-0.5">
                <span className="font-heading text-sm font-bold text-navy-DEFAULT">Personal Accident Cover</span>
                <span className="text-[10px] font-bold bg-life-DEFAULT text-white px-2 py-0.5 rounded-full">Mandatory</span>
              </div>
            ...
            <div className="text-right">
              ...
              <div className="w-5 h-5 bg-life-DEFAULT rounded-full flex items-center justify-center text-[9px] text-white ml-auto mt-1">✓</div>
            </div>
          </div>
            ...
            {RIDERS.map(r => (
              <div
                ...
                className={`border-[1.5px] rounded-2xl p-4 cursor-pointer transition-all duration-200 flex items-start gap-3.5 ${
                  activeRiders.includes(r.id) ? 'border-life-DEFAULT bg-life-bg' : 'border-slate-100 bg-white hover:border-life-DEFAULT'
                }`}
              >
                <div className="w-[38px] h-[38px] bg-life-bg rounded-xl flex items-center justify-center text-lg flex-shrink-0">{r.emoji}</div>
                <div className="flex-1">
                  <div className="font-heading text-sm font-bold text-navy-DEFAULT mb-0.5">{r.name}</div>
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  <div className={`w-5 h-5 border-2 rounded flex items-center justify-center text-[9px] text-white ml-auto mt-1.5 ${
                    activeRiders.includes(r.id) ? 'bg-life-DEFAULT border-life-DEFAULT' : 'border-slate-300'
      ...
      {screen === 9 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-4">
            <h4 className="font-heading text-sm font-bold text-navy-DEFAULT mb-4">📋 Policy Holder Details</h4>
                ...
                <div className="flex gap-2">
                  {(['Male', 'Female'] as const).map(g => (
                    <button key={g} onClick={() => setPropGender(g)}
                      className={`flex-1 py-3 rounded-xl text-sm font-bold border-[1.5px] transition-all cursor-pointer ${
                        propGender === g ? 'border-life-DEFAULT bg-life-bg text-life-dark' : 'border-slate-200 bg-white text-slate-500'
                      }`}>
                      {g === 'Male' ? '♂' : '♀'}
                    </button>
                  ))}
            ...
            <input type="text" placeholder="ABCDE1234F (optional)" value={propPan}
              ...
              className="w-full px-3.5 py-3 border-[1.5px] border-slate-200 rounded-xl font-body text-sm text-navy-DEFAULT outline-none focus:border-life-DEFAULT transition-all bg-white uppercase" />
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-5">
            <h4 className="font-heading text-sm font-bold text-navy-DEFAULT mb-4">🩺 Medical Declaration</h4>
            ...
            <div className="flex gap-2 mb-4">
              {[true, false].map(v => (
                <button key={String(v)} onClick={() => setOnMeds(v)}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-bold border-[1.5px] transition-all cursor-pointer ${
                    onMeds === v ? 'border-life-DEFAULT bg-life-bg text-life-dark' : 'border-slate-200 bg-white text-slate-500'
                  }`}>
                  {v ? 'Yes' : 'No'}
                </button>
              ))}
            ...
            <div className="flex gap-2">
              {[true, false].map(v => (
                <button key={String(v)} onClick={() => setHospitalized(v)}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-bold border-[1.5px] transition-all cursor-pointer ${
                    hospitalized === v ? 'border-life-DEFAULT bg-life-bg text-life-dark' : 'border-slate-200 bg-white text-slate-500'
                  }`}>
                  {v ? 'Yes' : 'No'}
                </button>
              ))}
          ...
          <div className="bg-white rounded-2xl shadow-s2 border border-slate-100 p-4 flex items-center justify-between">
            <div>
              ...
              <div className="font-heading text-xl font-extrabold text-navy-DEFAULT">₹{plan.base} <span className="text-xs text-slate-400 font-normal">/year</span></div>
            </div>
            <button
      ...
      {screen === 10 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <h2 className="font-heading text-[22px] font-extrabold text-navy-DEFAULT mb-1.5">Review Your Policy</h2>
          <p className="text-sm text-slate-500 mb-5">Please verify all details before proceeding to payment.</p>

          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden mb-4">
            <div className="bg-life-bg px-5 py-4 flex items-center gap-3 border-b border-life-DEFAULT/20">
              <div className="w-12 h-12 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-[11px] font-extrabold text-navy-DEFAULT flex-shrink-0">{plan.short}</div>
              <div>
                <div className="font-heading font-bold text-navy-DEFAULT text-sm">{plan.insurer}</div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-5">
            <div className="font-heading text-sm font-bold text-navy-DEFAULT mb-4">💰 Premium Breakdown</div>
            {[
            ...
            <div className="flex justify-between">
              <span className="font-heading font-bold text-navy-DEFAULT">Total Annual Premium</span>
      ...
      {screen === 11 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <p className="text-sm text-slate-500 mb-5">
            Total: <strong className="text-navy-DEFAULT">₹{total.toLocaleString('en-IN')}</strong> (annual, incl. GST)
          </p>
            ...
            {[
              ...
              <div key={i} onClick={() => setPayMethod(i)}
                className={`flex items-center gap-3.5 p-4 border-[1.5px] rounded-2xl cursor-pointer transition-all ${
                  payMethod === i ? 'border-life-DEFAULT bg-life-bg' : 'border-slate-100 bg-white hover:border-life-DEFAULT'
                }`}>
                ...
                <div className={`ml-auto w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
                  payMethod === i ? 'border-life-DEFAULT bg-life-DEFAULT' : 'border-slate-300'
                }`}>
          ...
          <button onClick={() => goTo(12)} className="w-full py-3.5 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-life-DEFAULT to-life-dark text-white hover:-translate-y-0.5 transition-transform">
      ...
      {screen === 12 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          {[
            ...
            <div key={item.key} onClick={() => item.set(true)}
              className={`border-2 rounded-2xl p-5 text-center cursor-pointer transition-all mb-4 ${
                item.state ? 'border-life-DEFAULT bg-life-bg' : 'border-dashed border-slate-300 bg-white hover:border-life-DEFAULT'
              }`}>
          ...
          <div className="flex items-start gap-2.5 text-[12.5px] text-slate-500 leading-snug mb-5">
            <input type="checkbox" checked={kycConsent} onChange={e => setKycConsent(e.target.checked)} className="mt-0.5 flex-shrink-0 accent-life-DEFAULT" />
          ...
          <div className="bg-white rounded-2xl shadow-s2 border border-slate-100 p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-sm font-bold text-navy-DEFAULT">{[kycFront, kycBack, kycPan].filter(Boolean).length} of 3 uploaded</div>
            </div>
            <button
      ...
      {screen === 14 && (
        <div className="max-w-[480px] mx-auto px-5 py-10">
          <div className="text-center mb-7">
            ...
            <div className="bg-life-bg border-2 border-life-DEFAULT/25 rounded-xl px-6 py-2 inline-block font-heading text-lg font-extrabold text-life-dark mb-6">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 max-w-xs mx-auto text-left mb-6">
              {[
                ...
                <div key={k as string} className={`flex justify-between py-1.5 border-b border-slate-100 last:border-0`}>
                  ...
                  <span className={`text-xs font-bold ${life ? 'text-life-DEFAULT' : 'text-navy-DEFAULT'}`}>{v}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-5 text-center mb-5">
            ...
            <h4 className="font-heading text-sm font-bold text-navy-DEFAULT mb-2">Term Life Insurance Policy</h4>
            ...
            <button
              ...
              className="w-full max-w-xs mx-auto block py-3 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-life-DEFAULT to-life-dark text-white hover:-translate-y-0.5 transition-transform"
            >
              📥 View Policy →
            </button>
          </div>
          ...
          <div className="text-center">
            <button onClick={() => showPage('home')} className="text-sm font-semibold text-life-DEFAULT bg-transparent border-none cursor-pointer hover:underline">
              ← Back to Home
            </button>
          </div>
        </div>
