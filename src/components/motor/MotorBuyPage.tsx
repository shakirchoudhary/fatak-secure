    ...
    <div className="pt-16 bg-slate-50 min-h-screen" ref={topRef}>
      ...
      <div className="bg-white border-b border-slate-100 px-[5%] py-3.5 flex items-center justify-between sticky top-16 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-heading font-bold text-navy-DEFAULT text-sm">Motor Insurance</div>
            <div className="text-[11px] text-slate-400">{STEP_LABELS[currentStep]} · Step {currentStep + 1} of 8</div>
          </div>
        </div>
        {regNo && screen >= 3 && (
          <div className="flex items-center gap-1.5 text-xs font-semibold text-navy-DEFAULT bg-motor-bg border border-motor-DEFAULT/20 px-3 py-1.5 rounded-xl">
      ...
      {screen === 0 && (
        <div className="max-w-[480px] mx-auto px-5 py-10 text-center">
          ...
          <h2 className="font-heading text-[24px] font-extrabold text-navy-DEFAULT mb-2">Every journey deserves protection.</h2>
          <p className="text-[13px] font-bold text-orange-DEFAULT mb-2">Buy Vehicle Insurance in 2 Minutes</p>
          ...
          <div className="bg-white rounded-2xl shadow-s1 p-6 text-left">
            ...
            <div className="flex items-center border-[1.5px] border-slate-200 rounded-xl overflow-hidden focus-within:border-motor-DEFAULT transition-all mb-1.5">
              ...
              <input
                ...
                className="flex-1 px-3.5 py-3 border-none outline-none font-body text-[14.5px] text-navy-DEFAULT bg-white uppercase"
              />
            </div>
            <button onClick={() => setRegNo('')} className="text-[12px] text-motor-DEFAULT font-semibold mb-4 bg-transparent border-none cursor-pointer p-0 underline-offset-2 hover:underline">
            ...
            <div className="flex items-center border-[1.5px] border-slate-200 rounded-xl overflow-hidden focus-within:border-motor-DEFAULT transition-all mb-4">
            ...
            <div className="flex items-start gap-2.5 text-xs text-slate-400 mb-5 leading-snug">
              <input type="checkbox" checked={consent} onChange={e => setConsent(e.target.checked)} className="mt-0.5 flex-shrink-0 accent-motor-DEFAULT" />
              <span>By submitting you agree to our <span className="text-motor-DEFAULT font-semibold cursor-pointer">Privacy Policy</span> and <span className="text-motor-DEFAULT font-semibold cursor-pointer">Terms & Conditions</span></span>
            </div>

            <button
              disabled={phone.length !== 10 || !consent}
              onClick={() => goTo(1)}
              className="w-full py-3.5 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-motor-DEFAULT to-motor-dark text-white disabled:opacity-40 disabled:cursor-not-allowed hover:-translate-y-0.5 transition-transform"
            >
              Insure Now →
            </button>
          </div>
            ...
            <div className="flex gap-3 justify-center mb-5">
              {otp.map((val, i) => (
                <input
                  ...
                  data-motp={i}
                  className="w-14 h-16 border-2 border-slate-200 rounded-xl text-center font-heading text-2xl font-extrabold text-navy-DEFAULT outline-none focus:border-motor-DEFAULT focus:shadow-[0_0_0_3px_rgba(245,158,11,0.12)] transition-all bg-white"
                />
              ))}
            </div>

            <p className="text-sm text-slate-400 mb-5">
              Didn't receive an OTP? <span className="text-motor-DEFAULT font-semibold cursor-pointer" onClick={() => setOtp(['', '', '', ''])}>Resend OTP</span>
            </p>
      ...
      {screen === 2 && (
        <div className="max-w-[480px] mx-auto px-5 py-24 text-center">
          <div className="relative w-20 h-20 mx-auto mb-6">
            ...
            <div className="absolute inset-0 rounded-full border-4 border-transparent border-t-motor-DEFAULT animate-spin" />
            <div className="absolute inset-0 flex items-center justify-center text-2xl">🚗</div>
          </div>
          <h3 className="font-heading text-xl font-extrabold text-navy-DEFAULT mb-2">Fetching your quotes...</h3>
        ...
        <div className="max-w-[480px] mx-auto px-5 py-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 px-4 py-3.5 flex items-center gap-3 mb-4">
            ...
            <div className="flex-1">
              <div className="font-heading text-sm font-bold text-navy-DEFAULT">{regNo || 'MH-01-CT-1376'} · Ford Ecosport</div>
              <div className="text-xs text-slate-400">Third Party Expiry: <strong>20 Oct 2025</strong></div>
            </div>
            <button onClick={() => goTo(0)} className="text-xs text-motor-DEFAULT font-semibold cursor-pointer bg-transparent border-none">✏️ Edit</button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-5">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 border-[1.5px] border-slate-200 rounded-xl font-body text-[13px] text-navy-DEFAULT outline-none focus:border-motor-DEFAULT transition-all bg-white"
              >
            ...
            <div>
              ...
              <select className="w-full px-3 py-2.5 border-[1.5px] border-slate-200 rounded-xl font-body text-[13px] text-navy-DEFAULT outline-none focus:border-motor-DEFAULT transition-all bg-white">
            ...
            {PLANS[planType].map((p, i) => (
              <div
                ...
                className={`bg-white border-[1.5px] rounded-[18px] p-5 cursor-pointer transition-all duration-200 relative ${
                  selectedPlan === i ? 'border-motor-DEFAULT shadow-s2' : 'border-slate-100 hover:border-motor-DEFAULT'
                }`}
              >
                {p.popular && (
                  <div className="absolute -top-3 left-4 bg-gradient-to-r from-orange-DEFAULT to-orange-dark text-white text-[10.5px] font-bold px-3 py-1 rounded-xl">⭐ Popular</div>
                )}
                <div className="flex items-center gap-3.5">
                  <div className="w-[50px] h-[50px] rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-[10px] font-extrabold text-navy-DEFAULT flex-shrink-0 text-center">
                    {p.name.split(' ')[0].toUpperCase()}<br />{p.name.split(' ')[1]?.toUpperCase()?.slice(0, 3) || ''}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-heading text-[14.5px] font-bold text-navy-DEFAULT">{p.name}</div>
                    <div className="text-xs text-slate-400">IDV: {p.idv} · {planType === 'comp' ? 'Comprehensive' : planType === 'od' ? 'Own Damage' : 'Third Party'}</div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="font-heading text-[22px] font-extrabold text-navy-DEFAULT leading-none">{p.price}</div>
                    <div className="text-xs text-slate-400">excl. GST</div>
                  </div>
                </div>
                {selectedPlan === i && (
                  <div className="mt-4 pt-4 border-t border-slate-100 flex gap-2">
                    <button className="flex-1 py-2 rounded-xl border-[1.5px] border-motor-DEFAULT text-motor-dark font-heading font-bold text-xs bg-white cursor-pointer">View Details</button>
                    <button
                      onClick={e => { e.stopPropagation(); goTo(4) }}
                      className="flex-1 py-2 rounded-xl border-none cursor-pointer font-heading font-bold text-xs bg-gradient-to-br from-motor-DEFAULT to-motor-dark text-white hover:-translate-y-0.5 transition-transform"
                    >
                      Buy Now →
                    </button>
                  </div>
      ...
      {screen === 4 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <h2 className="font-heading text-[22px] font-extrabold text-navy-DEFAULT mb-1.5">What is your previous<br />Own Damage expiry date?</h2>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden mb-4">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <button onClick={() => setCalDate(d => new Date(d.getFullYear(), d.getMonth() - 1, 1))}
                className="w-8 h-8 rounded-xl border border-slate-200 flex items-center justify-center text-slate-500 hover:border-motor-DEFAULT hover:text-motor-DEFAULT transition-all cursor-pointer bg-white font-bold">‹</button>
                  ...
                  return (
                    <button
                      ...
                      className={`h-9 rounded-xl text-sm font-semibold transition-all cursor-pointer border-none ${
                        isSelected ? 'bg-motor-DEFAULT text-white' : 'bg-transparent text-navy-DEFAULT hover:bg-motor-bg'
                      }`}
                    >
                      {day}
                    </button>
      ...
      {screen === 5 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <h2 className="font-heading text-[22px] font-extrabold text-navy-DEFAULT mb-1.5">Almost there!</h2>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 mb-5">
            <div className="font-heading text-sm font-bold text-navy-DEFAULT mb-3">Have you made claims last year?</div>
              ...
              {([['yes', '✅ Yes'], ['no', '❌ No'], ['ns', '🤔 Not sure']] as const).map(([v, label]) => (
                <button key={v} onClick={() => setMadeClaim(v)}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-bold border-[1.5px] transition-all cursor-pointer ${
                    madeClaim === v ? 'border-motor-DEFAULT bg-motor-bg text-motor-dark' : 'border-slate-200 bg-white text-slate-500'
                  }`}>
                  {label}
                </button>
              ))}
            ...
            <div className="font-heading text-sm font-bold text-navy-DEFAULT mb-3">Previous year No Claim Bonus %</div>
              ...
              {[0, 20, 25, 35, 45, 50].map(pct => (
                <button key={pct} onClick={() => setNcbPct(pct)}
                  className={`py-2.5 rounded-xl text-sm font-bold border-[1.5px] transition-all cursor-pointer ${
                    ncbPct === pct ? 'border-motor-DEFAULT bg-motor-DEFAULT text-white' : 'border-slate-200 bg-white text-navy-DEFAULT hover:border-motor-DEFAULT'
                  }`}>
                  {pct}
                </button>
              ))}
          ...
          <button
            onClick={() => goTo(6)}
            className="w-full py-3.5 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-motor-DEFAULT to-motor-dark text-white hover:-translate-y-0.5 transition-transform"
          >
            Continue →
          </button>
        </div>
      ...
      {screen === 6 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          <div className="bg-motor-bg rounded-xl px-4 py-3 flex items-center gap-3 mb-5 border border-motor-DEFAULT/20">
            <span className="text-lg">🏢</span>
            <div>
              <div className="font-heading text-sm font-bold text-navy-DEFAULT">{currentPlan?.name ?? 'ACKO Insurance'}</div>
          ...
          <h2 className="font-heading text-[22px] font-extrabold text-navy-DEFAULT mb-1.5">Pick your add-ons</h2>
          <p className="text-sm text-slate-500 mb-5">Customise your coverage. Add-ons are optional — proceed without selecting any.</p>

          {/* Mandatory PA */}
          <div className="bg-motor-bg border border-motor-DEFAULT/20 rounded-2xl p-4 flex items-start gap-3.5 mb-3">
            <div className="text-2xl flex-shrink-0">🧑‍⚕️</div>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-0.5">
                <span className="font-heading text-sm font-bold text-navy-DEFAULT">Personal Accident Cover</span>
                <span className="text-[10px] font-bold bg-motor-DEFAULT text-white px-2 py-0.5 rounded-full">Mandatory</span>
              </div>
            ...
            <div className="text-right">
              ...
              <div className="w-5 h-5 bg-motor-DEFAULT rounded-full flex items-center justify-center text-[9px] text-white ml-auto mt-1">✓</div>
            </div>
          </div>
            ...
            {MOTOR_ADDONS.map(a => (
              <div
                ...
                className={`border-[1.5px] rounded-2xl p-4 cursor-pointer transition-all duration-200 flex items-start gap-3.5 ${
                  activeAddons.includes(a.id) ? 'border-motor-DEFAULT bg-motor-bg' : 'border-slate-100 bg-white hover:border-motor-DEFAULT'
                }`}
              >
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  <div className={`w-5 h-5 border-2 rounded flex items-center justify-center text-[9px] text-white ml-auto mt-1.5 ${
                    activeAddons.includes(a.id) ? 'bg-motor-DEFAULT border-motor-DEFAULT' : 'border-slate-300'
      ...
      {screen === 7 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          <div className="bg-motor-bg rounded-xl px-4 py-3 flex items-center gap-3 mb-5 border border-motor-DEFAULT/20">
            ...
            <button onClick={() => null} className="text-[11px] text-motor-DEFAULT font-semibold cursor-pointer bg-transparent border-none">Policy brochure</button>
          </div>

          <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-5">
            <h4 className="font-heading text-[15px] font-extrabold text-navy-DEFAULT mb-4 pb-3 border-b-2 border-slate-100">Policy Holder Details</h4>
            ...
            <input type="text" placeholder="Enter full name" value={propName} onChange={e => setPropName(e.target.value)}
              className="w-full px-3.5 py-3 border-[1.5px] border-slate-200 rounded-xl font-body text-sm text-navy-DEFAULT outline-none focus:border-motor-DEFAULT transition-all bg-white mb-3.5" />
            ...
            <div className="grid grid-cols-2 gap-3 mb-3.5">
              <div>
                ...
                <input type="date" value={propDob} onChange={e => setPropDob(e.target.value)}
                  className="w-full px-3.5 py-3 border-[1.5px] border-slate-200 rounded-xl font-body text-sm text-navy-DEFAULT outline-none focus:border-motor-DEFAULT transition-all bg-white" />
              </div>
              <div>
                  ...
                  {(['Male', 'Female', 'Other'] as const).map(g => (
                    <button key={g} onClick={() => setPropGender(g)}
                      className={`flex-1 py-3 rounded-xl text-xs font-bold border-[1.5px] transition-all cursor-pointer ${
                        propGender === g ? 'border-motor-DEFAULT bg-motor-bg text-motor-dark' : 'border-slate-200 bg-white text-slate-500'
                      }`}>
                      {g === 'Male' ? '♂' : g === 'Female' ? '♀' : '⚧'}
                    </button>
                  ))}
          ...
          <div className="bg-white rounded-2xl shadow-s2 border border-slate-100 p-4 flex items-center justify-between">
            <div>
              ...
              <div className="font-heading text-xl font-extrabold text-navy-DEFAULT">₹{total.toLocaleString('en-IN')} <span className="text-xs text-slate-400 font-normal">incl. GST</span></div>
            </div>
            <button
              disabled={!propName || !propDob || !propEmail}
              onClick={() => goTo(8)}
              className="py-3 px-6 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-motor-DEFAULT to-motor-dark text-white disabled:opacity-40 disabled:cursor-not-allowed hover:-translate-y-0.5 transition-transform"
            >
              Continue →
            </button>
          </div>
      ...
      {screen === 8 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <h2 className="font-heading text-[22px] font-extrabold text-navy-DEFAULT mb-1.5">Summary</h2>
          <p className="text-sm text-slate-400 mb-6">{regNo || 'MH-01-CT-1376'} · Ford Ecosport</p>

          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden mb-4">
            <div className="bg-motor-bg px-5 py-4 flex items-center gap-3 border-b border-motor-DEFAULT/20">
              <div className="w-12 h-12 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-[10px] font-extrabold text-navy-DEFAULT flex-shrink-0 text-center">ACKO</div>
              <div>
                <div className="font-heading font-bold text-navy-DEFAULT text-sm">{currentPlan?.name ?? 'ACKO Insurance'}</div>
            ...
            <div className="px-5 py-4">
              {[
                ...
                <div key={k} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                  ...
                  <span className={`text-xs font-bold ${k === 'NCB Discount' ? 'text-emerald-500' : 'text-navy-DEFAULT'} text-right max-w-[55%]`}>{v}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-5">
            <div className="font-heading text-sm font-bold text-navy-DEFAULT mb-4 flex items-center gap-2"><span>💰</span> Premium Details</div>
            {[
              ...
              <div key={k} className={`flex justify-between text-sm py-1.5 ${k.includes('NCB') ? 'text-emerald-500' : ''}`}>
                ...
                <span className={`font-semibold ${k.includes('NCB') ? 'text-emerald-500' : 'text-navy-DEFAULT'}`}>{v}</span>
              </div>
            ))}
            <div className="border-t border-slate-100 my-2" />
            <div className="flex justify-between text-sm py-1.5">
              <span className="text-slate-500">Subtotal (excl. GST)</span>
              <span className="font-semibold text-navy-DEFAULT">₹{subtotal.toLocaleString('en-IN')}</span>
            </div>
            <div className="flex justify-between text-sm py-1.5">
            ...
            <div className="flex justify-between">
              <span className="font-heading font-bold text-navy-DEFAULT">Total Purchase Amount</span>
              <span className="font-heading text-lg font-extrabold text-navy-DEFAULT">₹{total.toLocaleString('en-IN')}</span>
            </div>
          </div>

          <button onClick={() => goTo(9)} className="w-full py-3.5 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-motor-DEFAULT to-motor-dark text-white hover:-translate-y-0.5 transition-transform">
            Proceed to Buy →
          </button>
        </div>
      )}
      ...
      {screen === 9 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          ...
          <p className="text-sm text-slate-500 mb-5">Total amount: <strong className="text-navy-DEFAULT">₹{total.toLocaleString('en-IN')}</strong> (incl. GST)</p>
            ...
            {[
              ...
              <div key={i} onClick={() => setPayMethod(i)}
                className={`flex items-center gap-3.5 p-4 border-[1.5px] rounded-2xl cursor-pointer transition-all ${
                  payMethod === i ? 'border-motor-DEFAULT bg-motor-bg' : 'border-slate-100 bg-white hover:border-motor-DEFAULT'
                }`}>
                ...
                <div className={`ml-auto w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
                  payMethod === i ? 'border-motor-DEFAULT bg-motor-DEFAULT' : 'border-slate-300'
                }`}>
          ...
          <div className="bg-slate-50 rounded-2xl p-4 flex items-center justify-between mb-5">
            <div>
              ...
              <div className="font-heading text-2xl font-extrabold text-navy-DEFAULT">₹{total.toLocaleString('en-IN')}</div>
            </div>
            <div className="text-right">
              <div className="text-xs text-slate-400">Order ID</div>
              <div className="text-[12.5px] font-semibold text-navy-DEFAULT">FS{Date.now().toString().slice(-10)}</div>
            </div>
          </div>

          <button onClick={() => goTo(10)} className="w-full py-3.5 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-motor-DEFAULT to-motor-dark text-white hover:-translate-y-0.5 transition-transform">
            🔒 Proceed to Pay →
          </button>
        </div>
      )}
      ...
      {screen === 10 && (
        <div className="max-w-[480px] mx-auto px-5 py-20 text-center">
          ...
          <div className="font-heading text-[26px] font-extrabold text-navy-DEFAULT mb-2">Payment Successful!</div>
          <p className="text-slate-600 text-sm mb-7">Your payment of <strong className="text-navy-DEFAULT">₹{total.toLocaleString('en-IN')}</strong> was successful!</p>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 max-w-xs mx-auto text-left mb-7">
            ...
            {[
              ...
              <div key={k} className="flex justify-between py-1.5 border-b border-slate-100 last:border-0">
                ...
                <span className={`text-xs font-bold ${k === 'Status' || k === 'Amount' ? 'text-emerald-500' : 'text-navy-DEFAULT'}`}>{v}</span>
              </div>
            ))}
          </div>

          <p className="text-sm text-slate-500 mb-6">
            Taking you to the next step in <span className="font-heading text-[22px] font-extrabold text-motor-DEFAULT">{countdown}</span>
          </p>

          <button onClick={() => goTo(11)} className="py-3.5 px-8 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-motor-DEFAULT to-motor-dark text-white hover:-translate-y-0.5 transition-transform mx-auto block max-w-xs w-full">
      ...
      {screen === 11 && (
        <div className="max-w-[480px] mx-auto px-5 py-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="font-heading text-[22px] font-extrabold text-navy-DEFAULT">KYC Verification Required</h2>
          </div>
          ...
          {[
            ...
            <div key={item.key} onClick={() => item.set(true)}
              className={`border-2 rounded-2xl p-5 text-center cursor-pointer transition-all mb-4 ${
                item.state ? 'border-motor-DEFAULT bg-motor-bg' : 'border-dashed border-slate-300 bg-white hover:border-motor-DEFAULT'
              }`}>
          ...
          <div className="flex items-start gap-2.5 text-[12.5px] text-slate-500 leading-snug mb-6">
            <input type="checkbox" checked={kycConsent} onChange={e => setKycConsent(e.target.checked)} className="mt-0.5 flex-shrink-0 accent-motor-DEFAULT" />
      ...
      {screen === 13 && (
        <div className="max-w-[480px] mx-auto px-5 py-10">
          <div className="text-center mb-8">
            ...
            <div className="inline-block bg-motor-bg border-2 border-motor-DEFAULT/30 rounded-xl px-6 py-2 font-heading text-lg font-extrabold text-motor-dark">
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-5 text-center mb-4">
            ...
            <h4 className="font-heading text-sm font-bold text-navy-DEFAULT mb-2">
            ...
            <button
              ...
              className="w-full max-w-xs mx-auto block py-3 rounded-2xl font-heading font-bold text-sm border-none cursor-pointer bg-gradient-to-br from-motor-DEFAULT to-motor-dark text-white hover:-translate-y-0.5 transition-transform"
            >
              📥 View Document
            </button>
          </div>
          ...
          <div className="text-center mt-4">
            <button onClick={() => showPage('home')} className="text-sm font-semibold text-motor-DEFAULT bg-transparent border-none cursor-pointer hover:underline">
              ← Back to Home
            </button>
          </div>
        </div>
